...
|---|---|---|---|
| D1 | `src/codetbl.cpp:874` (`CCodeTables::GetCodeName`) | one-byte overflow — a conversion name of exactly the caller's buffer length is copied whole and its terminator written one past the end | **Present.** `if (len > bufferLen)` still reads `>`; the callers pass 200-byte (`viewer3.cpp:58`, `:1914`) and 1024-byte (`dialogs3.cpp:136`) buffers. The longest shipped name is 33 bytes. The same function also copies the name into a fixed 1024-byte scratch buffer without a bound; the name comes from a user-editable configuration file (`convert\*\convert.cfg`) and is stored unbounded when the file is read |
...
- **D6 is worth one line, not a policy.** Node 24 is now installed on the development machine and the harness is green there; no repository workflow runs it. The feature makes the runner Node-major-independent because the cost is one line and the alternative — a note saying "needs Node 22.12+" — leaves the trap in place for the next machine. If the maintainer prefers the note, D6 is closed *verify-closed* with the note in the runner's header (FR-001 allows it).
