...
- [ ] T016 [US1] Proof-before D5: run [quickstart.md](quickstart.md) S5 on the unchanged tree and paste the heap-corruption report **As executed**: probe shows the 300-byte text overrunning `Text[MAX_PATH]` at +0; the heap-corruption run in the real comparator is still owed. **Same blocker** — needs a live `CFileHeaderWindow`, i.e. the File Comparator open under the debugger.
...
- [ ] T022 [US2] Proof-before D2: run [quickstart.md](quickstart.md) S2 and paste the Locals value (`0xCCCCCCCC`) reaching `SetMenuDefaultItem` **As executed**: not run — the state needs a debugger (T008). D2 rests on the static argument in research R2 plus the review. **Same blocker** — needs the Coding menu opened under the debugger.
...
- [ ] T028 [US2] Proof-before D3: run [quickstart.md](quickstart.md) S3 on the unchanged tree and paste the access violation **As executed**: the probe reproduces the fault (`exception 0xC0000005` reaching the name comparison) from the verbatim comparison shape; the in-process call at the site is still owed. **Same blocker** — the `.call` route is a dead end (recorded in the fix log); needs a live plugin call under the debugger.
